...
from ..utils.lora_metadata.analyzer import (
    analyze_lora_type,
...
def add_random_nsfw_command(subparsers, parent_parser):
    ...
    random_nsfw_parser.add_argument(
        "--use-vpred", action="store_true", help="Use v-prediction sampling"
    )
...
def get_random_style_loras(count=3):
    ...
    if len(style_loras) < count:
        logger.warning(
            f"Not enough style LoRAs found (only {len(style_loras)}), including some non-style LoRAs"
        )
        remaining_count = count - len(style_loras)
        non_style_loras = [
            lora for lora in lora_files if lora not in style_loras and lora not in EXCLUDED_LORAS
        ]
        if non_style_loras:
            additional_loras = random.sample(
                non_style_loras, min(remaining_count, len(non_style_loras))
            )
    ...
    if len(selected_loras) < count:
        logger.warning(
            f"Not enough unique artist LoRAs found (only {len(selected_loras)}), using fewer LoRAs than requested"
        )
...
def weighted_random_strength():
    ...
    selected_range = random.choices(
        LORA_STRENGTH_RANGES, weights=[r[2] for r in LORA_STRENGTH_RANGES], k=1
    )[0]
...
def generate_random_nsfw(args):
    ...
    for kink_name, kink_data in KINK_LORA_RECOMMENDATIONS.items():
        ...
        if has_kink and not args.checkpoint:
            logger.info(
                f"Detected {kink_name}-related kink. Recommending {kink_data['lora']} LoRA for better results."
            )
            logger.info(
                f"Example usage: --checkpoint {kink_data['checkpoint']} with {kink_data['lora']} LoRA"
            )
            logger.info(
                "Consider using a direct command instead of random-nsfw for better control: "
            )
            logger.info(
                f"cringegen nsfw-furry --lora {kink_data['lora']} --checkpoint {kink_data['checkpoint']}"
            )
    ...
    logger.info(
        f"Generating random NSFW prompts with species={generator.species}, gender={generator.gender}"
    )
                    ...
                    if image_path and len(image_path) > 0:
                        ...
                        if success:
                            ...
                            copied_path = os.path.join(
                                args.output_dir,
                                f"random_nsfw_{curr_seed}{os.path.splitext(image_path)[1]}",
                            )

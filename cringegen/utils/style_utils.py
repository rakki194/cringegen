        ...
        for related_mood in related_moods[mood]:
            if related_mood in STYLE_ATTRIBUTES["emotional_tones"]:
                matching_styles.extend(
                    STYLE_ATTRIBUTES["emotional_tones"][related_mood]["examples"]
                )
    ...
    for style in styles:
        ...
        if "visual_traits" in characteristics and characteristics["visual_traits"]:
            traits = random.sample(
                characteristics["visual_traits"], min(2, len(characteristics["visual_traits"]))
            )
        ...
        if (
            include_artists
            and "notable_artists" in characteristics
            and characteristics["notable_artists"]
        ):
...
def generate_style_prompt(
    styles: List[str], include_medium: bool = True, include_artists: bool = True
) -> str:
    """

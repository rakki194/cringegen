...
def select_random_lora(
    ...
    if tags:
        ...
        filtered_loras = {
            name: lora for name, lora in filtered_loras.items() if name in tag_filtered
        }
    ...
    if exclude_nsfw:
        filtered_loras = {
            name: lora
            for name, lora in filtered_loras.items()
...
def create_balanced_lora_set(
    ...
    for _ in range(character_count):
        if not character_loras:
            ...
            compatible = get_compatible_loras(
                LoRACategory.CHARACTER, existing_loras=style_loras, exclude_nsfw=exclude_nsfw
            )
            if compatible:
                character_loras.append(compatible[0])
            else:
                lora = select_random_lora(
                    category=LoRACategory.CHARACTER, exclude_nsfw=exclude_nsfw
                )
                if lora:
    ...
    for _ in range(concept_count):
        ...
        compatible = get_compatible_loras(
            LoRACategory.CONCEPT, existing_loras=existing, exclude_nsfw=exclude_nsfw
        )
        if compatible:
    ...
    for _ in range(pose_count):
        ...
        compatible = get_compatible_loras(
            LoRACategory.POSE, existing_loras=existing, exclude_nsfw=exclude_nsfw
        )
        if compatible:
...
def extract_lora_info_from_filename(filename: str) -> Dict[str, Any]:
    ...
    info = {
        ...
        "creator": None,
        "name": (
            filename.split("/")[-1].split("-")[0] if "/" in filename else filename.split("-")[0]
        ),
    }

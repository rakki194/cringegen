...
def test_prompt_analysis():
    ...
    for i, prompt in enumerate(prompts):
        ...
        print(
            f"  Section presence: {', '.join([k for k, v in analysis.section_presence.items() if v])}"
        )
...
def test_nltk_features():
    ...
    for i, text in enumerate(texts):
        ...
        for pos, words in pos_analysis.items():
            if words:
                print(
                    f"  {pos.capitalize()}: {', '.join(words[:5])}{'...' if len(words) > 5 else ''}"
                )
...
def test_text_generation():
    ...
    for case in test_cases:
        ...
        generator = FurryPromptGenerator(
            species=case["species"], gender=case["gender"], use_nlp=True, use_art_style=True
        )
        ...
        try:
            ...
            setting = SettingComponent(location=case["setting"], use_nlp_description=True)

            # Override the style
            style = StyleComponent(art_style=case["style"])
...
def test_integrated_workflow():
    ...
    print(
        "This test demonstrates how multiple NLP components can be used together in a realistic workflow."
    )
    ...
    print(
        f"Missing sections: {', '.join([k for k, v in analysis.section_presence.items() if not v])}"
    )
    ...
    print(
        f"Sentiment: Positive {sentiment['pos']:.2f}, Negative {sentiment['neg']:.2f}, Neutral {sentiment['neu']:.2f}"
    )
    ...
    final_prompt = enhance_prompt_with_background(
        anatomy_enhanced, location="forest", time_of_day="evening"
    )

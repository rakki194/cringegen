#!/usr/bin/env python3
"""
Test script for model architecture and family detection.
"""
...
# Try to import the function from cringegen, but provide a fallback for direct running
try:
    from cringegen.utils.logger import get_model_info, print_colored_warning
    DIRECT_MODE = False
except ImportError:
    # If import fails, use local implementations
    import re
    DIRECT_MODE = True
    ...
    def get_model_info(checkpoint_name: str) -> tuple:
        """Local implementation for direct execution."""
...
def test_model_detection():
    ...
    if DIRECT_MODE:
        print("\nNote: Running in direct mode with local implementations")
    else:
        print("\nNote: Using imported functions from cringegen.utils.logger")

def run_directly():
    """Run the test directly without importing the whole package."""

...
///     left_padding: 40,
///     font_size: Some(40.0),
///     horizontal_spacing: Some(10),
///     vertical_spacing: Some(10),
/// };
/// ```
#[derive(Debug)]
...
fn calculate_layout(config: &PlotConfig, max_width: u32, max_height: u32, cols: u32) -> Layout {
    ...
    // Get spacing from environment variables if set, otherwise use the config values or defaults
    let horizontal_spacing = match std::env::var("IMX_HORIZONTAL_SPACING") {
        Ok(val) => val.parse::<u32>().unwrap_or(0),
        Err(_) => config.horizontal_spacing.unwrap_or(5),
    };

    let vertical_spacing = match std::env::var("IMX_VERTICAL_SPACING") {
        Ok(val) => val.parse::<u32>().unwrap_or(0),
        Err(_) => config.vertical_spacing.unwrap_or(5),
    };
    ...
    let (max_row_label_width, _max_row_label_height) = if config.row_labels.is_empty() {
        ...
        config
            ...
            .fold((0.0f32, 0), |acc, (width, height)| {
                (acc.0.max(width), acc.1.max(height))
            })
    };
    ...
    // Calculate dimensions with spacing included
    let cell_width = max_width + horizontal_spacing;
    let cell_height = max_height + vertical_spacing;

    // Calculate total dimensions
    let content_width = (cell_width * cols) - horizontal_spacing; // Subtract trailing spacing
    let content_height = (cell_height * config.rows) - vertical_spacing; // Subtract trailing spacing

    let canvas_width = content_width + left_padding;
    let canvas_height = content_height + if has_labels { top_padding } else { 0 };
    ...
    for (i, img_path) in config.images.iter().enumerate() {
        ...
        // Calculate positions with spacing included
        let x_start = col * cell_width + left_padding;
        let y_start = row * cell_height + if has_labels { top_padding } else { 0 };
